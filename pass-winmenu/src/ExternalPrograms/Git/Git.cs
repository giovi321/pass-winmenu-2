...
namespace PassWinmenu.ExternalPrograms
{
	...
	internal class Git : IDisposable, ISyncService
	{
		...
		public void DeletePassword(string passwordFilePath)
		{
			CommitFile(passwordFilePath);
		}

		public void RenamePassword(string oldPasswordFilePath, string newPasswordFilePath)
		{
			// Stage the new path as well as the removal of the old path,
			// so the rename is recorded as a single commit.
			Commands.Stage(repo, newPasswordFilePath);
			Commands.Stage(repo, oldPasswordFilePath);

			var oldRelativePath = PathUtilities.MakeRelativePathForDisplay(repoDirectory, oldPasswordFilePath);
			var newRelativePath = PathUtilities.MakeRelativePathForDisplay(repoDirectory, newPasswordFilePath);
			var message = $"Rename password store file {oldRelativePath} to {newRelativePath}\n\n" +
						  "This commit was automatically generated by pass-winmenu.";
			CommitStagedChanges(message, newPasswordFilePath);
		}
		...
		private void CommitFile(string filePath)
		{
			...
			var message = $"{GetVerbFromGitFileStatus(status)} password store file {relativePath}\n\n" +
						  "This commit was automatically generated by pass-winmenu.";
			CommitStagedChanges(message, filePath);
		}

		private void CommitStagedChanges(string message, string logContext)
		{
			...
			if (sign?.Value ?? false)
			{
				Log.Send($"Generating GPG signature for {logContext}");
			...
			{
				Log.Send($"Generating unsigned commit for {logContext}");
